...
class GateKeeper
{
    ...
    {
        if ('settings_page_pollux' != get_current_screen()->id
    ...
    protected function getDependencyActions()
    {
        ...
        foreach ($this->errors as $plugin => $errors) {
            ...
            } elseif (in_array('wrong_version', $errors) && current_user_can('update_plugins')) {
                $actions .= $this->notice->updateButton($this->getPluginInformation($plugin));
            } elseif (in_array('inactive', $errors) && current_user_can('activate_plugins')) {
